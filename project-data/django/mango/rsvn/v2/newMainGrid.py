...
#=====================================================================
	def __init__(self,session):
		#-------------------------------------------
		self.dateStart = datetime.strptime(session['dateStart'],"%Y-%m-%d")
		self.dateEnd = datetime.strptime(session['dateEnd'],"%Y-%m-%d")
		self.numDays = (self.dateEnd - self.dateStart).days + 1
		self.rsvnid = session['rsvnid']
		self.rsvnListing = rsvn_select(self.dateStart, self.dateEnd,EXCLUDE_CANCEL)
		#-------------------------------------------
...
#--------------
	...
	#-------------------------------------------
		...
		for coll in self.collision :
			...
			'''.format(frontId,frontColor,lastId,lastColor) 
...
#			anchor = "<a href=/rsvn/grid/{} > O </a>".format(rsvn.id)
			anchor = "<button class='gbutt' value='{}' name='grid_id_select' > O </button>".format(rsvn.id)
			...
			for offset in range(span.days) :
				...
				for r in rsvn.roomset :
					if r.roominfo.type in  SORT_ORDER_LIST :
